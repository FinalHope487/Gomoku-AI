"""
hw1_11427234_v23.py — Deep & Stable Defense (v23: 深度優先穩定防守版)
針對分析報告改進：為了克服中盤複雜局面「超時算不完」造成的水平線效應瞎棋，
此版本繼承 v9 最穩定的 1.3 權重，但將候選著法 (MAX_CANDIDATES) 從 10 降到 8，
犧牲一點搜尋廣度，換取更深層的搜尋與更安全的時間管理 (Time Buffer 0.3s)。
"""
import sys
import time
...
MAX_DEPTH       = 7     
MAX_CANDIDATES  = 8     # 降至 8 以增加搜尋深度，克服複雜局面的中斷問題
QSEARCH_DEPTH   = 3     
NEIGHBOR_RADIUS = 2     
TIME_BUDGET     = 4.5   # 保證能在此時間內收斂，避免被主程式中斷
...
def evaluate(board, player: int) -> int:
    ...
    return mine - int(opp * 1.3)  # 全局固定 1.3 防禦權重
