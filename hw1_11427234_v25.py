"""
hw1_11427234_v25.py — Enhanced Dual-Track Defense (v25: 進化雙軌防禦版)
針對 v24 的改進版本：
1. TIME_BUDGET 調降為 4.2s，避免超時強制中斷導致回傳盲目走步。
2. 降低開局保底防禦至 1.35，減少評估函數過度飽和，提升 Alpha-Beta 剪枝效率。
3. 增加進攻傾向考量：若己方已有極大優勢（如即將連成四子或五子），則動態解除高防禦權重限制，主動進攻。
"""
import sys
import time
...
QSEARCH_DEPTH   = 3     
NEIGHBOR_RADIUS = 2     
TIME_BUDGET     = 4.2   # 從 4.5s 調降至 4.2s
...
def evaluate(board, player: int) -> int:
    ...
    # 增加進攻傾向的權衡：若己方已有絕佳反擊機會，解除保底防禦限制
    if mine >= 2_000_000 and opp < 10_000_000:
        # 自己即將獲勝(活四/雙死四等)，且對手沒有立即成五的威脅
        base_weight = min(_baseline_opp_weight, 0.8)
    elif mine >= 200_000 and opp < 200_000:
        # 自己有死四或好陣型，且對手無致命威脅
        base_weight = min(_baseline_opp_weight, 1.0)
    else:
        base_weight = _baseline_opp_weight

    # 最終防禦權重為兩者取大
    final_weight = max(base_weight, threat_weight)
...
def choose_move(board, my_color: int):
    ...
    if total_pieces < 15:
        _baseline_opp_weight = 1.35  # 從 1.5 調降至 1.35
    elif total_pieces <= 40:
        _baseline_opp_weight = 1.25  # 從 1.3 調降至 1.25
    else:
        _baseline_opp_weight = 1.15  # 從 1.2 調降至 1.15，以利反擊
