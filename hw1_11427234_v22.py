"""
hw1_11427234_v22.py — Early-Defense Phase-Adaptive (v22: 修正版開局高防禦階段適應)
針對分析報告改進：完全逆轉 v20 的防禦邏輯，解決開局速敗問題。
五子棋後手開局最弱勢，因此開局防禦權重必須最高（1.5）以遏止黑子先手。
隨著棋子增加，局勢穩定後逐漸調低防禦權重（中盤 1.3，殘局 1.2）以尋求己方反擊機會。
"""
import sys
import time
...
def choose_move(board, my_color: int):
    ...
    if total_pieces < 15:
        _current_opp_weight = 1.5   # 開局必須嚴密防守黑子擴張
    elif total_pieces <= 40:
        _current_opp_weight = 1.3   # 中盤攻守平衡
    else:
        _current_opp_weight = 1.2   # 殘局適度調低防禦以利反擊
